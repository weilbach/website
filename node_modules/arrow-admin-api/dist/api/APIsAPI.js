...
var API = require('./schema/API');
var GroupSummary = require('./schema/GroupSummary');
...
var APIsAPI = function (_APIProvider) {
	...
	_createClass(APIsAPI, null, [{
		...
		value: function bind(controller) {
			...
			swagger.schema('Status', Status.schema).schema('API', API.schema).schema('GroupSummary', GroupSummary.schema);
   ...
   */
			...
			swagger.get('/apis/:group').operationId('getGroupEndpoints').tag('apis').summary('Gets an array of Endpoints for a group.').parameter({ in: 'path', name: 'group', type: 'string', description: 'API group to filter by.' }).response(200, 'OK', 'API', true).response(404).response(500).action(function (req, res) {
				var opts = {};
