...
var Timeline = function () {
	...
	_createClass(Timeline, [{
		...
		value: function process(obj) {
			try {
				if (obj && obj.type) {
					...
					// TODO: return error or something else?
					// we shouldn't have problems without letting the UI know.
					console.error('not sure how to handle', obj);
				}
			} catch (err) {
		...
		// A flow is not a module component so needs custom processing.

	}, {
		key: 'processFlow',
		value: function processFlow(timeline, logs, metadata, args, result, error) {
			var tok = timeline.name.match(/^(.*?):(.*)/);
			timeline.name = tok.length === 3 ? tok.slice(2)[0] : timeline.subname;
			delete timeline.subname;
			var flow = this.arrow.getFlow(timeline.name);

			timeline.detail = {};
			timeline.detail.args = args;
			timeline.detail.result = result;
			timeline.detail.source = JSON.stringify(flow, null, 2);
						...
						this.timeline.timeline.forEach(function (timeline) {
							...
							try {
								switch (timeline.type) {
									...
									case 'flow':
										{
											this.processFlow(timeline, logs, metadata, args, result, error);
											break;
										}
									case 'action':
										{
