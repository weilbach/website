...
function getObjectModel(appc, opts, projectdir, callback) {
	var Arrow = appc.arrow;
	var cwd = process.cwd();
	var server = new Arrow({
		port: 0,
		dir: projectdir,
		overrideLevel: opts.logLevel === 'info' ? 'error' : opts.logLevel,
		admin: {
			enabled: false
		}
	});
...
function generateModelForProject(appc, opts, callback) {
	...
	async.waterfall([
		...
		function (cb) {
			getObjectModel(appc, opts, projectdir, function (err, om) {
				if (err) {
					return cb(err);
				}
				objectModel = om;
