...
module.exports = {
	...
	fields: [
		{
			flags: '--force',
			description: 'DEPRECATED. The SDK generator will be removed in a future release. For more information please see: https://docs.axway.com/bundle/API_Builder_allOS_en/page/api_builder_tools_3_1_0_release_note.html',
			type: 'confirm',
			name: 'force',
			default: false,
			message: 'DEPRECATED. The SDK generator will be removed in a future release. For more information please see: https://docs.axway.com/bundle/API_Builder_allOS_en/page/api_builder_tools_3_1_0_release_note.html. \nAre you sure you want to continue?'
		},
		{
			flags: '--sdktype <sdktype>',
			description: 'set the SDK type. can be one of ' + _.map(SDKTYPES, 'value').map(injectOr).join(', '),
			type: 'list',
			name: 'sdktype',
			when: (opts => opts.force),
...
function generate(appc, args, opts, callback) {
	...
	if (_.filter(SDKTYPES, { value: opts.sdktype }).length === 0) {
		return callback(new Error('"' + opts.sdktype + '" is not a recognized sdktype; please instead use one of the following: ' + _.map(SDKTYPES, 'value').map(injectOr).join(', ') + '.'));
	}
	...
	server.start(function (err) {
		...
		objectModel.port = new Arrow({ dir: dir }, true).port;
		...
		// Security.
		objectModel.authByBasic = objectModel.config.APIKeyAuthType === 'basic';
		objectModel.authByAPIKey = objectModel.config.APIKeyAuthType === 'apikey';
		objectModel.authByPlugin = objectModel.config.APIKeyAuthType === 'plugin';
		...
		for (var modelName in models) {
			if (models.hasOwnProperty(modelName)) {
				...
				objectModel.models.push({
					...
					endpoints: endpoints,
					baseURL: endpoints && _.map(endpoints, 'path').sort()[0]
				});
			}
		}
		...
		if (code.indexOf('/** filename:') >= 0) {
			...
			for (var j = 1; j < splits.length; j++) { // Note: We skip the first (blank) split.
				...
				if (filename.indexOf('/') >= 0) {
					fs.ensureDirSync(path.join(opts.sdkdir, filename.split('/').slice(0, -1).join('/')));
				}
	...
	function translateEndpoint(endpoint) {
		...
		var params = endpoint.parameters,
			pathParams = _.filter(params, { type: 'path' }),
			queryParams = _.filter(params, { type: 'query' }),
			bodyParams = _.filter(params, { type: 'body' });

		endpoint.args = [];
		endpoint.argNames = [];
		endpoint.isGET = endpoint.method === 'GET';
		endpoint.safeName = safeName(endpoint.method + '/' + endpoint.path);

		// Path parameters become their own full argument.
		if (Object.keys(pathParams).length > 0) {
			var pathNames = _.map(pathParams, 'name');
