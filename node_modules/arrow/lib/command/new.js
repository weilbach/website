...
New.prototype.run = function run() {
	var cli = this.cli,
		program = cli.program,
		args = arguments;
...
New.prototype.generate = function generate(opts, callback) {
	...
	var pkg = this.cli.pkg,
		...
		defaults = {
			...
			org_id: '',
			author: '',
			name_lc: opts.name.toLowerCase()
		},
		filter = merge(defaults, opts);
		...
		if (isJSON.test(fn) && fs.existsSync(to)) {
			...
			try {
				...
				var toJSON = JSON.parse(toBuf),
					toResult = merge(toJSON, fromJSON);
